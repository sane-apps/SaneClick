...
struct AppStoreNativeActionTests {
    ...
    @Test("requiresNativeRuntime matches the natively-executed actions")
    func requiresNativeRuntimeMatchesNativeExecutedActions() throws {
        // `requiresNativeRuntime` no longer means "native-only"; it means "runs
        // through the native engine on BOTH builds." That set is the 9 native-only
        // actions (OCR/PDF/copy-path variants, no shell equivalent) PLUS the 10
        // native-preferred image actions (they have a sips equivalent but we run
        // native for non-destructive, consistent behavior).
        ...
        let nativePreferredImages: Set<AppStoreNativeAction> = [
            .convertToPNG,
            .convertToJPEG,
            .heicToJPEG,
            .resize50,
        ...
        let nativeExecuted = nativeOnly.union(nativePreferredImages)

        #expect(nativeOnly.count == 9)
        #expect(nativePreferredImages.count == 10)
        #expect(nativeExecuted.count == 19)
        for action in AppStoreNativeAction.allCases {
            #expect(action.requiresNativeRuntime == nativeExecuted.contains(action))
            // Every native-runtime action must resolve to a matching native action
            // via init(script:) so the direct build can actually route to it.
            if action.requiresNativeRuntime {
                let library = try #require(ScriptLibrary.libraryScript(named: action.rawValue))
                #expect(AppStoreNativeAction(script: library.toScript()) == action)
            }
        }
    }
