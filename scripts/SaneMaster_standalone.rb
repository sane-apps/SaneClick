...
XCODEPROJ = File.join(PROJECT_ROOT, "SaneClick.xcodeproj")
SCHEME = "SaneClick"
...
def test_mode
  build
  app = Dir.glob(File.join(PROJECT_ROOT, "build/**/SaneClick.app")).first
  app ||= Dir.glob(File.expand_path("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/SaneClick.app")).first
  abort "Could not find built SaneClick.app" unless app
  system("killall SaneClick 2>/dev/null")
  sleep 0.5
  system("open '#{app}'")
  warn "Launched #{app}"
end
