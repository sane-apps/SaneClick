...
- 2026-07-27 **1.3.2 direct release candidate cleared on the Mini.**
  ...
  - Accessibility is granted for `com.saneclick.SaneClick` (`auth_value=2`). Release-mode launch passed with receipt `5e3d95085cb6b09898450ef6028c93d4`.
  - The customer UI sweep parser now suppresses nested SaneMaster workflow-receipt noise before parsing JSON. A focused source guard covers the fix.
  - Canonical full verify passed 178 tests in 22 suites: receipt `c2dbe22ff461288891f689130c8a5438`.
  - Fresh customer UI sweep passed all eight action families: receipt `00802fa9f5b265e0ab937a8469d65d50`; strict visual contract passed: receipt `a51e9a2521af40776e76a8cbd069c44c`.
  ...
  - Finder’s automation tree exposed the selected fixture row but not the context menu itself; the real Finder click was not claimed. Clearance relies on the current Mini launch/extension state, representative action execution tests, strict current receipt, and prior clean Finder proof.
...
- 2026-06-29 (END OF SESSION, now resolved ↑) **▶ was: RESUME HERE — blocked push.** Everything customer-facing is DONE + LIVE.
  - **DONE + LIVE (nothing pending here):** 1.2.0 / 1.2.1 / 1.3.0 all shipped to every channel incl. **Lemon Squeezy** (1.3.0 build `1300`; owner confirmed the LS upload + post-release check passed). saneclick.com has the OCR-led hero copy, the OCR grouped-menu hero screenshot, the **53→62** action-count fix, AND a **new sourced competitor comparison table** (named: iBoysoft MagicMenu / Service Station / FilePane — every cell verified against each vendor's live page June 2026, footnoted with source links; leads on OCR + no-subscription, does NOT claim price leadership since FilePane is cheaper). #6 (garrattg) got an owner-approved "it's out, please try it" reply.
  - **THE ONLY UNRESOLVED THING (pure git/gate plumbing, not customer-facing):** several LOCAL commits on the Air's `main` are ahead of `origin` and have NOT pushed: `c773693` (handoff), the `.saneprocess` App Store prep, `1e88978` (53→62), `cbbc675` (competitor table), `c4030c2` (the FIX below). The push is blocked by a RESEARCH-REQUIRED circuit breaker.
  - **Root cause — ALREADY FIXED:** rewriting the `.saneprocess` `appstore:` comment deleted the literal phrase **"direct download only"**, which `AppStoreReviewGuardrailTests` → "Inactive App Store lane documents direct download strategy" asserts must be present → `verify` failed. Fixed in `c4030c2` (restored "Current live strategy is direct download only."). ⚠️ My earlier "render test / locked screen" diagnoses were WRONG and wasted many turns — that render test PASSES; I misread parallel Swift-Testing output (the failing test ran early; the render test was just the last one *shown* before the run-failed summary). **Lesson: read the actual error first** — `xcrun xcresulttool get test-results summary --path <latest .xcresult>` showed the real failure in one step. (See [[verify-output-content-not-shape]].)
  - **TO RESUME (next session, fresh = breaker reset):** run `SANEMASTER_FORCE_LOCAL=1 ./scripts/SaneMaster.rb verify` — it should PASS now (the fix is in, render test passes). Then `git push origin main` to land the backlog, and reconcile the Mini. If the breaker is still tripped, invoke the gate certifier (ARCHITECTURE.md → ADR-011) with the proof: the fix restores the exact asserted string, and the breaker's apple-docs-research demand does not apply to a config-text fix. The breaker's own UNTRACKED log files (`.claude/gate-hammer-log.jsonl`, `.claude/gate-hits.json`) are noise tripping the Rule #4 Stop hook — don't commit them (transient) or delete them (resets the breaker); gitignore if they keep flagging.
  - **Still owner-gated (App Store track, unchanged):** flip `appstore.enabled`, regenerate 2880×1800 screenshots, the owner's Pro image-action spot-check, then `appstore_submit.rb`.
